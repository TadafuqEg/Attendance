...
@section('title', 'Dashboard - permission requests')
@section('content')	
<style>
    .pagination{
        display: inline-flex;
                    ...
                    <div class="card">
                      <div class="card-body">
                        <div>
                            ...
                            <form id="searchForm" class="search-bar" style="margin-bottom:1%;margin-right:20px;margin-left:0px;"method="post" action="{{ route('permission_requests') }}" enctype="multipart/form-data">
                                @csrf
                                <div style="display:flex;">
                                  <h5 class="card-title" style="width: 65%;">Permission Requests</h5>
                        ...
                        <table class="table table-hover">
                          ...
                          <tbody>
                            ...
                            @foreach($all_requests as $leave_request)
                              <tr>
                                ...
                                <td>{{$leave_request->from?date('h:i a',strtotime($leave_request->from)):' '}}</td>
                                <td>{{$leave_request->to?date('h:i a',strtotime($leave_request->to)):' '}}</td>
                                ...
                                <td>
                                  ...
                                  <a href="{{url('/admin-dashboard/permission_request/edit/'.$leave_request->id)}}" style="margin-right: 1rem;">
                                    <span  class="bi bi-pen" style="font-size: 1rem; color: rgb(255,255,255);"></span>
                                  </a>
                                 
                                  {{-- <a href="{{url('/admin-dashboard/permission_request/delete/'.$leave_request->id)}}">
                          ...
                          @else
                              <tr>
                                <td>There are no Permission Requests.</td>
                              </tr>
                          @endif
                          </tbody>
                        </table>
                        <div style="text-align: center;">
                          {!! $all_requests->appends(['search' => request('search'),'user'=>request('user'),'date_from'=>request('date_from'),'date_to'=>request('date_to'),'status'=>request('status')])->links("pagination::bootstrap-4") !!}
                        </div>
                      </div>
                      </div>
                    </div>
